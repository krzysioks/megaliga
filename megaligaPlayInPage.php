<?php
/*
Template Name: Megaliga Playin
Description: Shows playin schedule and scoreboard of given round for one group in the ligue
 */
...
<div class="<?php echo hestia_layout(); ?>">
                    ...
                    <article id="post-<?php the_ID(); ?>" class="section section-text">
                        <div class="row">
                            ...
                            if ($_POST['submitScore']) {
                                ...
                                function getDatabaseActionOnRow($getPlayersIdQuery, $playerKeyList, $idStartingLineup)
                                {
                                    ...
                                    foreach ($getPlayersIdQuery as $row) {
                                        ...
                                        foreach ($row as $col) {
                                            ...
                                            if ($col) {
                                                $checkIfExistQuery = $wpdb->get_results('SELECT id_scores FROM megaliga_scores_playin WHERE id_schedule = ' . $_POST['id_schedule'] . ' AND id_starting_lineup = ' . $_POST[$idStartingLineup] . ' AND id_player = ' . $col);
                                        ...
                                        foreach ($row as $col) {
                                            if ($col) {
                                                ...
                                                if ($selectedAction[$playerKeyList[$i]]) {
                                                    ...
                                                    $wpdb->update('megaliga_scores_playin', $submitDataArray, $where);
                                                } else {
                                                    $wpdb->insert('megaliga_scores_playin', $submitDataArray);
                                                }
                                            }
                                            $i++;
                                        }
                                ...
                                function postTrainerScore($teamNumber, $selectedAction, $keysToSubmitArray, $ID)
                                {
                                    ...
                                    if ($selectedAction) {
                                        ...
                                        $wpdb->update('megaliga_trainer_score_playin', $submitDataArray, $where);
                                    } else {
                                        $wpdb->insert('megaliga_trainer_score_playin', $submitDataArray);
                                    }
                                ...
                                $getTeam1PlayersIdQuery = $wpdb->get_results('SELECT player1, player2, player3, player4, player5 FROM megaliga_starting_lineup_playin WHERE id_starting_lineup = ' . $_POST['id_starting_lineup_team1']);

                                //get players' id for team2
                                $getTeam2PlayersIdQuery = $wpdb->get_results('SELECT player1, player2, player3, player4, player5 FROM megaliga_starting_lineup_playin WHERE id_starting_lineup = ' . $_POST['id_starting_lineup_team2']);
                                ...
                                $checkIfExistTrainerTeam1Query = $wpdb->get_results('SELECT id_trainer FROM megaliga_trainer_score_playin WHERE id_schedule = ' . $_POST['id_schedule'] . ' AND ID = ' . $_POST['id_user_team1']);

                                $checkIfExistTrainerTeam2Query = $wpdb->get_results('SELECT id_trainer FROM megaliga_trainer_score_playin WHERE id_schedule = ' . $_POST['id_schedule'] . ' AND ID = ' . $_POST['id_user_team2']);
                                ...
                                $wpdb->update('megaliga_schedule_playin', array('team1_score' => $team1Outcome), $where);
                                $wpdb->update('megaliga_schedule_playin', array('team2_score' => $team2Outcome), $where);
                            }

                            // handle playOff schedule generation
                            if ($_POST['submitPlayOffSchedule']) {
                                ...
                                for ($i = 0; $i < 5; $i++) {
                                    $matchupData = array('id_user_team1' => 0, 'id_user_team2' => 0, 'scoreTeam1Round1' => 0, 'scoreTeam2Round1' => 0, 'scoreTeam1Round2' => 0, 'scoreTeam2Round2' => 0, 'totalScoreTeam1' => null, 'totalScoreTeam2' => null, 'seedNumberTeam1' => 0, 'seedNumberTeam2' => 0, 'winner' => 'none', 'winnerTotalWinsNumber' => 0, 'winnerTotalScore' => 0);

                                    $matchupData['id_user_team1'] = $getSchedulePlayin[$i]->id_user_team1;
                                    $matchupData['id_user_team2'] = $getSchedulePlayin[$i]->id_user_team2;
                                ...
                                // get seed numbers for teams advancing to playoff
                                ...
                                foreach ($playinData as $matchup) {
                                    if ($matchup['winner'] == 'team1') {
                                        array_push($winnersData, array('id_user' => $matchup['id_user_team1'], 'winnerTotalWinsNumber' => $matchup['winnerTotalWinsNumber'], 'winnerTotalScore' => $matchup['winnerTotalScore']));

                                        array_push($luckyLooserData,  array('id_user' => $matchup['id_user_team2'], 'scoreGapToWinner' => $matchup['totalScoreTeam1'] - $matchup['totalScoreTeam2']));
                                    } else if ($matchup['winner'] == 'team2') {
                                        array_push($winnersData, array('id_user' => $matchup['id_user_team2'], 'winnerTotalWinsNumber' => $matchup['winnerTotalWinsNumber'], 'winnerTotalScore' => $matchup['winnerTotalScore']));

                                        array_push($luckyLooserData,  array('id_user' => $matchup['id_user_team1'], 'scoreGapToWinner' => $matchup['totalScoreTeam2'] - $matchup['totalScoreTeam1']));
                                    }
                                }
                                ...
                                $checkIfRecordsExists = $wpdb->get_results('SELECT id_schedule, round_number FROM megaliga_schedule_playoff');
                                if (count($checkIfRecordsExists) > 0) {
                                    ...
                                    foreach ($checkIfRecordsExists as $record) {
                                        ...
                                        foreach ($winnersData as $data) {
                                            if ($winnerIterator == $j) {
                                                $winnerUserId = $data['id_user'];
                                            }

                                            if ($loserIterator == $j) {
                                                $loserUserId = $data['id_user'];
                                            }
                                        ...
                                        $submitDataArray = array();
                                        $submitDataArray['id_user_team1'] = $winnerUserId;
                                        $submitDataArray['id_user_team2'] = $loserUserId;
                                        $submitDataArray['round_number'] = $record->round_number;
                                        $submitDataArray['team1_score'] = null;
                                        $submitDataArray['team2_score'] = null;
                                        $submitDataArray['team1_seed'] = $winnerIterator + 1;
                                        $submitDataArray['team2_seed'] = $loserIterator + 1;
                                    ...
                                    $loserIterator = 3;
                                    for ($winnerIterator = 0; $winnerIterator < 2; $winnerIterator++) {
                                        $winnerUserId = 0;
                                        $loserUserId = 0;
                                        $i = 0;
                                ...
                                for ($i = 0; $i < 4; $i++) {
                                    ...
                                    $where = array('ID' => $winnersData[$i]['id_user']);
                                    $wpdb->update('megaliga_user_data', $submitDataArray, $where);
                                }
                            ...
                            function drawScoreBoard($scoreBoardData, $userId)
                            {
                                ...
                                $getScoreDataTeam1Query = $wpdb->get_results('SELECT heat1, heat2, heat3, heat4, heat5, heat6, heat7, setplays, comment FROM megaliga_scores_playin WHERE id_schedule = ' . $scoreBoardData['id_schedule'] . ' AND id_starting_lineup = ' . $idStartingLineup . ' ORDER BY starting_order ASC');

                                //get score for team1 trainer
                                $getScoreDataTeam1TrainerQuery = $wpdb->get_results('SELECT heat1, heat2, heat3, heat4, heat5, heat6, heat7, setplays, comment FROM megaliga_trainer_score_playin WHERE id_schedule = ' . $scoreBoardData['id_schedule'] . ' AND ID = ' . $scoreBoardData['id_user_team1']);
                                ...
                                $getScoreDataTeam2Query = $wpdb->get_results('SELECT heat1, heat2, heat3, heat4, heat5, heat6, heat7, setplays, comment FROM megaliga_scores_playin WHERE id_schedule = ' . $scoreBoardData['id_schedule'] . ' AND id_starting_lineup = ' . $idStartingLineup2 . ' ORDER BY starting_order ASC');

                                //get score for team2 trainer
                                $getScoreDataTeam2TrainerQuery = $wpdb->get_results('SELECT heat1, heat2, heat3, heat4, heat5, heat6, heat7, setplays, comment FROM megaliga_trainer_score_playin WHERE id_schedule = ' . $scoreBoardData['id_schedule'] . ' AND ID = ' . $scoreBoardData['id_user_team2']);
                            ...
                            function getAllGameData($query, $round_number)
                            {
                                ...
                                foreach ($query as $gameField) {
                                    ...
                                    $getTeam1StartingLineupQuery = $wpdb->get_results('SELECT id_starting_lineup, player1, player2, player3, player4, player5, setplays FROM megaliga_starting_lineup_playin WHERE megaliga_starting_lineup_playin.ID = ' . $gameField->id_user_team1 . ' AND megaliga_starting_lineup_playin.round_number = ' . $round_number);
                                    ...
                                    $getTeam2StartingLineupQuery = $wpdb->get_results('SELECT id_starting_lineup, player1, player2, player3, player4, player5, setplays FROM megaliga_starting_lineup_playin WHERE megaliga_starting_lineup_playin.ID = ' . $gameField->id_user_team2 . ' AND megaliga_starting_lineup_playin.round_number = ' . $round_number);
                            ...
                            //get teams that reached playin stage for given round
                            $getSchedule4PlayinTeam1 = $wpdb->get_results('SELECT megaliga_team_names.name as "team_name", megaliga_schedule_playin.team1_score, megaliga_schedule_playin.id_user_team1, megaliga_user_data.logo_url FROM megaliga_user_data, megaliga_team_names, megaliga_schedule_playin WHERE megaliga_user_data.ID = megaliga_schedule_playin.id_user_team1 AND megaliga_user_data.team_names_id = megaliga_team_names.team_names_id AND megaliga_schedule_playin.round_number = ' . $round_number);

                            $getSchedule4PlayinTeam2 = $wpdb->get_results('SELECT megaliga_team_names.name as "team_name", megaliga_schedule_playin.team2_score, megaliga_schedule_playin.id_user_team2, megaliga_user_data.logo_url FROM megaliga_user_data, megaliga_team_names, megaliga_schedule_playin WHERE megaliga_user_data.ID = megaliga_schedule_playin.id_user_team2 AND megaliga_user_data.team_names_id = megaliga_team_names.team_names_id AND megaliga_schedule_playin.round_number = ' . $round_number);

                            //get data for the scoreboard
                            //get all games for playin for given round
                            $getGames4Playin = $wpdb->get_results('SELECT id_schedule, id_user_team1, id_user_team2 FROM megaliga_schedule_playin WHERE round_number = ' . $round_number);
                            $scoreBoradPlayinData = getAllGameData($getGames4Playin, $round_number);

                            //content of the megaliga page
                            the_content();

                            //show button to generate schedule for playoff for semifinal stage only if user with ID == 14 (mbaginski) || 48 (Gabbana) and round_number == 2
                            if (($userId == 14 || $userId == 48) && $round_number == 2) {
                                ...
                                $scoresAdded = false;
                                foreach ($getPlayOffScores as $record) {
                            ...
                            drawSchedule($getSchedule4PlayinTeam1, $getSchedule4PlayinTeam2, $getGames4Playin, 'left', $round_number);
                            ...
                            echo '  </div>';
                            foreach ($scoreBoradPlayinData as $gameData) {
    ...
    <script type="text/javascript">
        (function() {
            ...
            title.innerHTML = 'playins - ' + title.innerHTML;
        })();
    </script>
    <?php get_footer(); ?>