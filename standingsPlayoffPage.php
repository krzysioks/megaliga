<?php
/*
Template Name: StandingsPlayoff
Description: Standings page which includes playoff phase and playoff scores influence on regular season standings
 */
?>
<?php get_header(); ?>
<main id="content">
...
//function which take one game from schedule and add's it outcome to overall standings array ($standingsData)
function addGames($scheduleQuery, $standingsData)
{
    foreach ($scheduleQuery as $game) {
        if ($game->team1_score != null) {
            //addGames function may be used to add playoff games outcomes. Check if $standingsData (standings for given ligue group) array includes given id of team1 and team2
            $isIdUserTeam1Present = array_key_exists($game->id_user_team1, $standingsData);
            $isIdUserTeam2Present = array_key_exists($game->id_user_team2, $standingsData);

            if ($isIdUserTeam1Present) {
                $standingsData[$game->id_user_team1]['totalScore'] = $standingsData[$game->id_user_team1]['totalScore'] + $game->team1_score;
            }

            if ($isIdUserTeam2Present) {
                $standingsData[$game->id_user_team2]['totalScore'] = $standingsData[$game->id_user_team2]['totalScore'] + $game->team2_score;
            }

            //when team1 wins
            if ($game->team1_score > $game->team2_score) {
                //verify that id of team is in $standingsData
                if ($isIdUserTeam1Present) {
...
function calculateStandingsData($scheduleQuery, $schedulePlayoff, $userIDquery)
{
    $standingsData = array();
    ...
    //adding game score to standings based on regular season. Do not add games if array is empty
    if (sizeof($scheduleQuery)) {
        $standingsData = addGames($scheduleQuery, $standingsData);
    }
    
    //adding game scores from playoff phase to standings. Only teams, that reached playoff stage will be affected. Do not add games if array is empty
    if (sizeof($schedulePlayoff)) {
        $standingsData = addGames($schedulePlayoff, $standingsData);
    }
...
function drawStandings($standings, $side, $groupName)
{
    ...
    $title = $groupName == 'playoff' ? 'Playoff' : 'Grupa ' . ucfirst($groupName);
    echo '<table class="scheduleTable ' . $margin . '" border="0">';
    echo '  <tr><td colspan="8" class="scheduleTableName textLeft">' . $title . '</td></tr>';
    echo '  <tr>
...
$getUserIDPlayoffQuery = $wpdb->get_results('SELECT ID FROM megaliga_user_data WHERE reached_playoff = 1');
...
$getSchedule4playoffQuery = $wpdb->get_results('SELECT team1_score, team2_score, id_user_team1, id_user_team2 FROM megaliga_schedule_playoff');

$standingsDolce = calculateStandingsData($getSchedule4DolceQuery, $getSchedule4playoffQuery, $getUserIDDolceQuery);

$standingsGabbana = calculateStandingsData($getSchedule4GabbanaQuery, $getSchedule4playoffQuery, $getUserIDGabbanaQuery);

$standingsPlayoff = calculateStandingsData(array(), $getSchedule4playoffQuery, $getUserIDPlayoffQuery);
...
drawStandings($standingsPlayoff, 'left', 'playoff');
echo '  </div>';
echo '</div>';
?>
